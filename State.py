...
def checkIfGameIsWon (state: np.array) -> int:
    ...
    for i in range(m):
        for j in range(n - 3):
            if (np.sum(state[i, n : n + 4]) == 4.0): return 1 
    ...
    if (np.sum(np.abs(state[0, :])) == float(n)): return 0 
...
def makeMove (state: np.array, idx: int) -> np.array:
    ...
    for i in range(1, height): # Play moves from the button up
        print(i)
...
def getAvailableMoves(state: np.array) -> np.array:
    ...
    result = np.zeros((1, width)) # TODO: Make sure that the dimensions of the output array matches 
    for i in range(width):
        if (state[height - 1][i] == 0): result[0][i] = 1.0
    return result